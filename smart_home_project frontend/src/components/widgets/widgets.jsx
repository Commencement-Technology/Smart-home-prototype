...
function widgets(){
    ...
    useEffect(()=>{
        ...
        try{
            ...
            if (error.response && error.response.status === 400) {
                seterror(new Error("Location not found. Please enter a valid location."));
              } else {
                seterror(error);
              }
        }
    ...
    // const adjustedTime = new Date(time.getTime() + cityTimeZoneOffset * 60 * 1000);
        ...
        return (
            <div className={s.container}>
                ...
                <p className={s.locationname}>Weather in: {weatherdata.location.country}, {weatherdata.location.name}</p>
                ...
                <div className={s.temp_container}>
                    ...
                    <svg stroke="currentColor" fill="currentColor" strokeWidth="0" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 30 30" className="h-6 w-6 text-white" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M9.91,19.56c0-0.85,0.2-1.64,0.59-2.38s0.94-1.35,1.65-1.84V5.42c0-0.8,0.27-1.48,0.82-2.03S14.2,2.55,15,2.55
	...
	c-0.43-0.48-0.96-0.77-1.58-0.85V9.69c0-0.06-0.03-0.13-0.1-0.19c-0.07-0.07-0.14-0.1-0.22-0.1c-0.09,0-0.16,0.03-0.21,0.08
	                c-0.05,0.06-0.08,0.12-0.08,0.21v7.34c-0.61,0.09-1.13,0.37-1.56,0.85C12.66,18.37,12.45,18.92,12.45,19.56z"></path></svg>
                    </div>
                    <p className={s.temp}><span className={s.tempp}>Temperature</span> 
                    <br/><span className={s.temp}>{weatherdata.current.temp_c}°C</span></p>
                </div>

                <div className={s.temp_container}>
                    <div className={s.tempicon}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" className="bi bi-wind" viewBox="0 0 16 16"> <path d="M12.5 2A2.5 2.5 0 0 0 10 4.5a.5.5 0 0 1-1 0A3.5 3.5 0 1 1 12.5 8H.5a.5.5 0 0 1 0-1h12a2.5 2.5 0 0 0 0-5zm-7 1a1 1 0 0 0-1 1 .5.5 0 0 1-1 0 2 2 0 1 1 2 2h-5a.5.5 0 0 1 0-1h5a1 1 0 0 0 0-2zM0 9.5A.5.5 0 0 1 .5 9h10.042a3 3 0 1 1-3 3 .5.5 0 0 1 1 0 2 2 0 1 0 2-2H.5a.5.5 0 0 1-.5-.5z"/> </svg>
                    </div>
                    <p className={s.temp}><span className={s.tempp}>Wind Speed</span> 
                    <br/><span className={s.temp}>{weatherdata.current.wind_kph} km/h</span></p>
                </div>
